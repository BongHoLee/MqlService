...
public class LessThanOperation extends RelationalOperation {

    public LessThanOperation(MQLOperand leftOperand, MQLOperand rightOperand) {
    ...
    protected List<Map<String, Object>> operate(ColumnOperand leftOperand, ColumnOperand rightOperand, Map<String, List<Map<String, Object>>> mqlDataSource) {
        ...
        leftDataSource.forEach(leftRow -> rightDataSource.forEach(rightRow -> {
            ...
            if (operand.lessThan(rightRow.get(rightOperand.getExpressionToString()))){
        ...
        List<Map<String, Object>> result = leftDataSource.stream()
                .filter(
                        eachRow -> {
                            ...
                            return operand.lessThan(rightOperand);
                        }
                )
        ...
        List<Map<String, Object>> result = leftDataSource.stream()
                .filter(
                        eachRow -> {
                            ...
                            return rightOperand.lessThan(rightColumnOperandValue);
                        }
                )
                .collect(Collectors.toList());
