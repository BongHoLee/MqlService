...
public class LargerThanOrEqualToOperation extends RelationalOperation {

    public LargerThanOrEqualToOperation(MQLOperand leftOperand, MQLOperand rightOperand) {
    ...
    protected List<Map<String, Object>> operate(ColumnOperand leftOperand, ColumnOperand rightOperand, Map<String, List<Map<String, Object>>> mqlDataSource) {
        ...
        leftDataSource.forEach(leftRow -> rightDataSource.forEach(rightRow -> {
            ...
            if (operand.largerThanOrEqualTo(rightRow.get(rightOperand.getExpressionToString()))){
        ...
        List<Map<String, Object>> result = leftDataSource.stream()
                .filter(
                        eachRow -> {
                            ...
                            return operand.largerThanOrEqualTo(rightOperand);
                        }
                )
        ...
        List<Map<String, Object>> result = leftDataSource.stream()
                .filter(
                        eachRow -> {
                            ...
                            return rightOperand.largerThanOrEqualTo(rightColumnOperandValue);
                        }
                )
                .collect(Collectors.toList());
