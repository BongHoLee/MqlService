...
public class LessThanOrEqualToOperation extends RelationalOperation {

    public LessThanOrEqualToOperation(MQLOperand leftOperand, MQLOperand rightOperand) {
    ...
    protected List<Map<String, Object>> operate(ColumnOperand leftOperand, ColumnOperand rightOperand, Map<String, List<Map<String, Object>>> mqlDataSource) {
        ...
        leftDataSource.forEach(leftRow -> rightDataSource.forEach(rightRow -> {
            ...
            if (operand.lessThanOrEqualTo(rightRow.get(rightOperand.getExpressionToString()))){
        ...
        List<Map<String, Object>> result = leftDataSource.stream()
                .filter(
                        eachRow -> {
                            ...
                            return operand.lessThanOrEqualTo(rightOperand);
                        }
                )
        ...
        List<Map<String, Object>> result = leftDataSource.stream()
                .filter(
                        eachRow -> {
                            ...
                            return rightOperand.lessThanOrEqualTo(rightColumnOperandValue);
                        }
                )
                .collect(Collectors.toList());
